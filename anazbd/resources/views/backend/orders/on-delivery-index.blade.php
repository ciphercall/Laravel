    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            @forelse($chalans as $key => $chalan)
                {{--  @dd($chalan->getOriginal(),$chalan->order->getOriginal())  --}}
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td>{{ $chalan->order->delivery_agent != null ? $chalan->order->delivery_agent->name : "N/A" }}</td>

                    <td>
                        <div class="btn-group btn-group-mini btn-corner">
                            <a href="{{route('backend.order.on-delivery.show', $chalan->id)}}"
                            ...
                            <button type="button" class="btn btn-xs btn-danger"
                                    onclick="delete_check({{$chalan->order->id}})"
                                    title="Delete">
                                <i class="ace-icon fa fa-trash-o"></i>
                            </button>
                        </div>
                        {{--  <form action="{{ route('backend.order.destroy', $order->id)}}"
                              id="deleteCheck_{{ $chalan->order->id }}" method="GET">
                            @csrf
                        </form>  --}}
                    </td>
                </tr>
