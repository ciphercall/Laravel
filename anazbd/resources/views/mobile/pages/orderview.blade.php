...
{{--    <!--breadcrumbs area end-->--}}
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-12 overflow-auto" style="padding-left: 0px !important;">
                {{-- @foreach ($histories as $key => $history) --}}
                @php
                    $orderstatus = ['Created','Accepted','Pickup From Seller','Arrived at Warehouse',
                        'Quality Checking',
                        'Packing','On Delivery','Delivered'];
                    $isCancelled = false;
                @endphp
                <div class="md-stepper-horizontal orange">
                    @foreach ($orderstatus as $status)
                        @if($histories->contains('type',$status))
                            <div class="col md-step active" style="padding: 5px;">
                                <div class="md-step-circle" >
                                    <span>
                                        <i class="fa fa-check" aria-hidden="true"></i>
                                    </span>
                                </div>
                                <div class="md-step-title" style="font-size: small">{{ $status == 'Quality Checking' ? 'Quality Checked' : $status }}</div>
                                <div class="md-step-bar-left"></div>
                                <div class="md-step-bar-right"></div>
                            </div>
                        @elseif($histories->contains('type','Cancelled') && !$isCancelled)
                            @php
                                $isCancelled = true;
                            @endphp
                            <div class="col md-step active" style="padding: 5px;">
                        ...
                        @else
                            <div class="col md-step inactive" style="padding: 5px;">
                                <div class="md-step-circle">
                                  <span>
                                      <i class="fa fa-check" aria-hidden="true"></i>
                                  </span>
        ...
        @foreach ($histories as $history)
            <div class="row">
                <div class="col-3">
                    <p style="float: left; font-size: medium; font-weight: bold"> {{ Carbon\Carbon::parse($history->time)->format('Y-m-d h:i A') }}
                    </p>
{{--                    <p>{{ Carbon\Carbon::parse($history->time)->format('Y-m-d') }} <br>--}}
{{--                        {{ Carbon\Carbon::parse($history->time)->format('H : i a') }}</p>--}}
                </div>
                    ...
                    <div class="timeline">
                        <div class="container-timeline right">
                            <div class="content" style="padding: 10px !important;">
                                <h5 style="font-size: 12pt;">Order {{ $history->type == 'Quality Checking' ? 'Quality Checked' : $history->type }} </h5>
                                <p>Your Order {{ $history->type == 'Quality Checking' ? 'Quality Checked' : $history->type }} Successfully.</p>
                            </div>
                        </div>
                    </div>
                </div>
