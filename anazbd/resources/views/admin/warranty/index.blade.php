    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            @forelse($warranties as $key => $item)
                                <tr>
                                    <td>{{($warranties->total()-$loop->index)-(($warranties->currentpage()-1) * $warranties->perpage())}}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>
                                        <div class="btn-group btn-group-minier btn-corner">
                                            ...
                                            <button id="editBtn" class="btn btn-xs btn-warning" data-name="{{ $item->name }}" data-id="{{ $item->id }}"><i class="fa fa-edit"></i></button>
                                        ...
                                        <form action="{{ route('backend.product.warranty-types.destroy', $item->id)}}"
        ...
        <div class="col-4">
            <div class="card rounded shadow">
                ...
                <div class="card-body">
                    <form id="form" action="{{ route('backend.product.warranty-types.store') }}" method="POST">
                        @csrf
                        @if(old('id') > 0)
