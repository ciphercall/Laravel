...
class NotDeliveredController extends Controller
{
    public function index(Request $request)
    {
        $users = Agent::whereHas('invoices', function ($q) {
            $q->where('status', 'Not Delivered')
                ->select('id', 'order_id', 'status');
        })
            ->select(['id', 'name'])
            ->get();
        $invoices = Chalan::where('status','Not Delivered')
        ...
        return view('admin.orders.not-delivered.index',compact('invoices','users'));
    }
    ...
    public function show(Chalan $chalan)
    {
        ...
        return view('admin.orders.not-delivered.show',compact(['chalan','data']));
    }

    public function edit(Chalan $chalan)
    {
        $chalan->load('order','order.items','billing_address','chalan_items');
        $data = PendingController::getAddresses($chalan->order->billing_address->division_id,$chalan->order->billing_address->city_id);
        return view('admin.orders.not-delivered.edit',compact(['order','data']));
    }
